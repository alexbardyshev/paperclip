  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">Фінансова книга</CardTitle>
        <CardDescription>
          Нарахування на рівні рахунку, що не прив'язані до конкретного запиту.
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          ...
          subtitle={`${eventCount} подій всього за діапазон`}
          icon={ArrowUpRight}
        />
        <MetricTile
          label="Кредити"
          value={formatCents(creditCents)}
          subtitle="Повернення, компенсації та кредитні відшкодування"
          icon={ArrowDownLeft}
        />
        <MetricTile
          label="Нетто"
          value={formatCents(netCents)}
          subtitle="Дебет мінус кредит за обраний період"
          icon={ReceiptText}
        />
        <MetricTile
          label="Оцінка"
          value={formatCents(estimatedDebitCents)}
          subtitle="Орієнтовні дебети, що ще не підтверджені рахунком"
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message="Оберіть компанію, щоб переглянути витрати." />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">Витрати</h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  Витрати на інференс, платформні збори, кредити та поточні квотні вікна.
                </p>
            </div>
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label="Витрати на інференс"
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={`${formatTokens(inferenceTokenTotal)} токенів по запитах`}
              icon={DollarSign}
            />
            <MetricTile
              label="Бюджет"
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? `${budgetData?.pausedAgentCount ?? 0} агентів призупинено · ${budgetData?.pausedProjectCount ?? 0} проєктів призупинено`
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? `${formatCents(spendData.summary.spendCents)} of ${formatCents(spendData.summary.budgetCents)}`
                    : "Місячний ліміт не налаштовано"
              }
              icon={Coins}
            />
            <MetricTile
              label="Фінансове нетто"
              value={formatCents(financeData?.summary.netCents ?? 0)}
              subtitle={`${formatCents(financeData?.summary.debitCents ?? 0)} дебетів · ${formatCents(financeData?.summary.creditCents ?? 0)} кредитів`}
              icon={ReceiptText}
            />
            <MetricTile
              label="Фінансові події"
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={`${formatCents(financeData?.summary.estimatedDebitCents ?? 0)} оцінка за діапазон`}
              icon={ArrowUpRight}
            />
          </div>
      </div>
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          ...
          <TabsTrigger value="budgets">Бюджети</TabsTrigger>
          <TabsTrigger value="providers">Провайдери</TabsTrigger>
          <TabsTrigger value="billers">Білери</TabsTrigger>
          <TabsTrigger value="finance">Фінанси</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">Оберіть дати початку та кінця для завантаження даних.</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">Книга інференсу</CardTitle>
                    <CardDescription>
                      Витрати на інференс за запитами для обраного періоду.
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4 px-5 pb-5 pt-2">
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      ...
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">Використання</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {spendData.summary.utilizationPercent}% щомісячного бюджету витрачено за цей діапазон.
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">За агентом</CardTitle>
                    <CardDescription>Що витратив кожен агент за обраний період.</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">Подій витрат ще немає.</p>
                    ) : (
                          ...
                          <div key={row.agentId} className="border border-border px-4 py-3">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  вх {formatTokens(row.inputTokens + row.cachedInputTokens)} · вих {formatTokens(row.outputTokens)}
                                </div>
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">За проєктом</CardTitle>
                      <CardDescription>Витрати запусків, атрибутовані через задачі проєкту.</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">Витрат за проєктами ще немає.</p>
                      ) : (
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage="Фінансових подій ще немає. Додайте нарахування на рівні рахунку, коли надійдуть рахунки або кредити від білера." />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">Панель керування бюджетом</CardTitle>
                  <CardDescription>
                    Жорсткі ліміти витрат для агентів і проєктів. Квоти підписки провайдера ведуться окремо і відображаються у вкладці Провайдери.
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    ...
                    subtitle="Відкриті м'які або жорсткі перевищення порогу"
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label="Очікують затвердження"
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle="Запити на перевищення бюджету очікують рішення"
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label="Призупинені агенти"
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle="Сигнали агентів заблоковані бюджетом"
                    icon={Coins}
                  />
                  <MetricTile
                    label="Призупинені проєкти"
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle="Виконання проєктів заблоковано бюджетом"
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">Активні інциденти</h2>
                    <p className="text-sm text-muted-foreground">
                      Вирішіть жорсткі зупинки тут, підвищивши бюджет або явно залишивши область призупиненою.
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">{scopeType === "company" ? "Бюджети компанії" : scopeType === "agent" ? "Бюджети агентів" : "Бюджети проєктів"}</h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? "Щомісячна політика для всієї компанії."
                            : scopeType === "agent"
                              ? "Регулярні щомісячні ліміти витрат для окремих агентів."
                              : "Довічні ліміти витрат для виконуваних проєктів."}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      Бюджетних політик ще немає. Встановіть бюджети агентів і проєктів на їхніх сторінках деталей або скористайтеся наявним щомісячним контролем бюджету компанії.
                    </CardContent>
                  </Card>
                ) : null}
              </div>
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Подій витрат у цьому періоді немає.</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Платних подій у цьому періоді немає.</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr,0.95fr]">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">За білером</CardTitle>
                      <CardDescription>Фінансові події на рівні рахунку, згруповані за тим, хто їх нарахував або кредитував.</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">Фінансових подій ще немає.</p>
                      ) : (
