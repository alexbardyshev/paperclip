...
const codexThinkingEffortOptions = [
  { id: "", label: "Авто" },
  { id: "minimal", label: "Мінімальне" },
  { id: "low", label: "Низьке" },
  { id: "medium", label: "Середнє" },
  { id: "high", label: "Високе" },
] as const;
...
const openCodeThinkingEffortOptions = [
  ...
  { id: "medium", label: "Середнє" },
  { id: "high", label: "Високе" },
  { id: "max", label: "Максимальне" },
] as const;

const cursorModeOptions = [
  { id: "", label: "Авто" },
  { id: "plan", label: "Планування" },
  { id: "ask", label: "Запит" },
] as const;
    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {isDirty && !props.hideInlineSave && (
        <div className="sticky top-0 z-10 flex items-center justify-end px-4 py-2 bg-background/90 backdrop-blur-sm border-b border-primary/20">
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted-foreground">Незбережені зміни</span>
            <Button
              size="sm"
              onClick={handleSave}
              disabled={!isCreate && props.isSaving}
            >
              {!isCreate && props.isSaving ? "Збереження..." : "Зберегти"}
            </Button>
          </div>
        </div>
      )}
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">Особистість</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">Особистість</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label="Ім'я" hint={help.name}>
              <DraftInput
            ...
            <Field label="Посада" hint={help.title}>
              <DraftInput
                value={eff("identity", "title", props.agent.title ?? "")}
                onCommit={(v) => mark("identity", "title", v || null)}
                immediate
                className={inputClass}
                placeholder="напр. VP of Engineering"
              />
            </Field>
            <Field label="Підпорядкується" hint={help.reportsTo}>
              <ReportsToPicker
                agents={companyAgents}
                value={eff("identity", "reportsTo", props.agent.reportsTo ?? null)}
                onChange={(id) => mark("identity", "reportsTo", id)}
                excludeAgentIds={[props.agent.id]}
                chooseLabel="Вибрати менеджера…"
              />
            </Field>
            <Field label="Можливості" hint={help.capabilities}>
              <MarkdownEditor
                value={eff("identity", "capabilities", props.agent.capabilities ?? "")}
                onChange={(v) => mark("identity", "capabilities", v || null)}
                placeholder="Опишіть, що цей агент може робити..."
            ...
            {isLocal && !props.hidePromptTemplate && (
              <>
                <Field label="Шаблон запиту" hint={help.promptTemplate}>
                  <MarkdownEditor
                    value={eff(
                      "adapterConfig",
                      "promptTemplate",
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                  Шаблон запиту відтворюється при кожному пульсі. Тримайте його компактним і динамічним, щоб уникнути повторних витрат токенів і скидання кешу.
                </div>
              </>
            )}
          </div>
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          {cards
            ? <h3 className="text-sm font-medium">Адаптер</h3>
            : <span className="text-xs font-medium text-muted-foreground">Адаптер</span>
          }
          {showAdapterTestEnvironmentButton && (
            ...
            >
              {testEnvironment.isPending ? "Тестування..." : "Тест середовища"}
            </Button>
          )}
        </div>
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          ...
          {showLegacyWorkingDirectoryField && (
            <Field label="Робоча директорія (застаріла)" hint={help.cwd}>
          ...
          {isLocal && isCreate && (
            <>
              ...
              <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                Шаблон запиту відтворюється при кожному пульсі. Надавайте перевагу малому формулюванню задачі та змінним, таким як <code>{"{{ context.* }}"}</code> або <code>{"{{ run.* }}"}</code>; уникайте повторення стабільних інструкцій тут.
              </div>
            </>
          )}
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">Дозволи та конфігурація</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">Дозволи та конфігурація</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              <Field label="Команда" hint={help.localCommand}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.command
              ...
              {fetchedModelsError && (
                <p className="text-xs text-destructive">
                  {fetchedModelsError instanceof Error
                    ...
                    : "Не вдалося завантажити моделі адаптера."}
                </p>
              )}
                  ...
                  {adapterType === "codex_local" &&
                    ...
                    currentThinkingEffort === "minimal" && (
                      <p className="text-xs text-amber-400">
                        Codex може відхилити `minimal` мислення, коли увімкнено пошук.
                      </p>
                    )}
                </>
              )}
              {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
                <>
                  <Field label="Bootstrap-запит (застарілий)" hint={help.bootstrapPrompt}>
                    <MarkdownEditor
                      value={eff(
                        "adapterConfig",
                      ...
                      placeholder="Необов'язковий початковий запит для першого запуску"
                      contentClassName="min-h-[44px] text-sm font-mono"
                      imageUploadHandler={async (file) => {
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
                    Bootstrap-запит є застарілим і буде видалений у майбутньому релізі. Розгляньте можливість переміщення цього вмісту до шаблону запиту агента або файлу інструкцій.
                  </div>
                </>
              )}
              {adapterType === "claude_local" && (
              ...
              <Field label="Додаткові аргументи (через кому)" hint={help.extraArgs}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.extraArgs
              ...
              <Field label="Змінні середовища" hint={help.envVars}>
                <EnvVarEditor
                  value={
                    isCreate
              ...
              {!isCreate && (
                <>
                  <Field label="Тайм-аут (сек)" hint={help.timeoutSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
                  ...
                  <Field label="Пільговий період переривання (сек)" hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
      ...
      {isCreate && showCreateRunPolicySection ? (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium flex items-center gap-2 mb-3"><Heart className="h-3 w-3" /> Політика запуску</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground flex items-center gap-2"><Heart className="h-3 w-3" /> Політика запуску</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              label="Пульс за інтервалом"
              ...
              numberLabel="sec"
              numberPrefix="Запускати пульс кожні"
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <CollapsibleSection
              title="Розширена політика запуску"
              bordered={cards}
            ...
            <div className="space-y-3">
              ...
              />
              <Field label="Час охолодження (сек)" hint={help.cooldownSec}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
              ...
              <Field label="Макс. паралельних запусків" hint={help.maxConcurrentRuns}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "maxConcurrentRuns",
...
function AdapterEnvironmentResult({ result }: { result: AdapterEnvironmentTestResult }) {
  const statusLabel =
    result.status === "pass" ? "Пройдено" : result.status === "warn" ? "Попередження" : "Помилка";
  const statusClass =
    result.status === "pass"
      ...
      <div className="mt-2 space-y-1.5">
        {result.checks.map((check, idx) => (
          <div key={`${check.code}-${idx}`} className="text-[11px] leading-relaxed break-words">
            ...
            {check.hint && <span className="block opacity-90 break-words">Підказка: {check.hint}</span>}
          </div>
        ))}
      </div>
    </div>
      ...
      <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
        {ADAPTER_DISPLAY_LIST.map((item) => (
          ...
          >
            ...
            {item.comingSoon && (
              <span className="text-[10px] text-muted-foreground">Незабаром</span>
            )}
          </button>
        ))}
      </PopoverContent>
...
function EnvVarEditor({
  ...
  async function sealRow(i: number) {
    ...
    try {
      ...
      setSealError(err instanceof Error ? err.message : "Не вдалося створити секрет");
    }
  }
  ...
  return (
    <div className="space-y-1.5">
          ...
          <div key={i} className="flex items-center gap-1.5">
            ...
            {row.source === "secret" ? (
              <>
                ...
                >
                  <option value="">Вибрати секрет...</option>
                  {secrets.map((secret) => (
                ...
                <button
                  ...
                  title="Створити секрет з поточного значення"
                >
                  Новий
                </button>
              </>
              ...
              <>
                ...
                <button
                  ...
                  onClick={() => sealRow(i)}
                  disabled={!row.key.trim() || !row.plainValue}
                  title="Зберегти значення як секрет та замінити посиланням"
                >
                  Запечатати
                </button>
              </>
      ...
      <p className="text-[11px] text-muted-foreground/60">
        Змінні PAPERCLIP_* автоматично вводяться під час виконання.
      </p>
    </div>
  );
}
      ...
      >
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected
                ? selected.label
                : value || (allowDefault ? "За замовчуванням" : required ? "Вибрати модель (обов'язково)" : "Вибрати модель")}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          ...
          <div className="max-h-[240px] overflow-y-auto">
            ...
            {modelSearch.trim() && !models.find((m) => m.id === modelSearch.trim()) && (
              <button
                className="flex items-center w-full px-2 py-1.5 text-sm rounded hover:bg-accent/50 border-t border-border mt-1 pt-2"
                onClick={() => {
                  onChange(modelSearch.trim());
                  onOpenChange(false);
                }}
              >
                <span className="block w-full text-left truncate text-muted-foreground">
                  Використати: <span className="text-foreground font-mono">{modelSearch.trim()}</span>
                </span>
              </button>
            )}
            {filteredModels.length === 0 && !modelSearch.trim() && (
              <p className="px-2 py-1.5 text-xs text-muted-foreground">Моделей не знайдено.</p>
            )}
          </div>
        </PopoverContent>
      </Popover>
...
function ThinkingEffortDropdown({
  ...
  return (
    <Field label="Зусилля мислення" hint={help.thinkingEffort}>
