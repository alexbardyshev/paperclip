...
const quickFilterPresets = [
  { label: "Всі", statuses: [] as string[] },
  { label: "Активні", statuses: ["todo", "in_progress", "in_review", "blocked"] },
  { label: "Бклог", statuses: ["backlog"] },
  { label: "Виконано", statuses: ["done", "cancelled"] },
];
      ...
      label:
        key === "__unassigned"
          ...
          : key.startsWith("__user:")
            ? (formatAssigneeUserLabel(key.slice("__user:".length), currentUserId) ?? "Користувач")
  ...
  return (
    <div className="space-y-4">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="sm" className={`text-xs ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`}>
                ...
                <span className="hidden sm:inline">{activeFilterCount > 0 ? `Фільтри: ${activeFilterCount}` : "Фільтр"}</span>
                {activeFilterCount > 0 && (
            ...
            <PopoverContent align="end" className="w-[min(480px,calc(100vw-2rem))] p-0">
              <div className="p-3 space-y-3">
                ...
                <div className="space-y-1.5">
                  <span className="text-xs text-muted-foreground">Швидкі фільтри</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
                  ...
                  <div className="space-y-3">
                    ...
                    {/* Assignee */}
                    <div className="space-y-1">
                      <span className="text-xs text-muted-foreground">Виконавець</span>
                      <div className="space-y-0.5 max-h-32 overflow-y-auto">
                        <label className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                          ...
                          />
                          <span className="text-sm">Без виконавця</span>
                        </label>
                        {currentUserId && (
                          <label className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                            <Checkbox
                    ...
                    {labels && labels.length > 0 && (
                      <div className="space-y-1">
                        <span className="text-xs text-muted-foreground">Мітки</span>
                        <div className="space-y-0.5 max-h-32 overflow-y-auto">
                          {labels.map((label) => (
                    ...
                    {projects && projects.length > 0 && (
                      <div className="space-y-1">
                        <span className="text-xs text-muted-foreground">Проєкт</span>
                        <div className="space-y-0.5 max-h-32 overflow-y-auto">
      ...
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message="Задач не знайдено за поточними фільтрами або пошуком."
                          ...
                          >
                            {issue.assigneeAgentId && agentName(issue.assigneeAgentId) ? (
                              ...
                              <span className="inline-flex items-center gap-1.5 text-xs">
                                ...
                                {formatAssigneeUserLabel(issue.assigneeUserId, currentUserId) ?? "Користувач"}
                              </span>
                            ) : (
